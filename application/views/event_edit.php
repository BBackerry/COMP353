...
<div class="row">
	...
	<div class="panel-body">
		<form role="form" class="form-horizontal" name="editEvent" method="post" action="<?= site_url('Event/submitEditEvent/') ?>">
				...
				<div class="col-lg-10">
					<select multiple class="form-control" name="setProgramChair[]" data-validate="required" size="10">
						<?php foreach($users as $m):    
							...
							<option value="<?= $m->idUser ?>" <?= $selected ? 'selected' : '' ?>><?= $m->idUser ?> - <?= $m->lastName ?>, <?= $m->firstName ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
			...
			<div class="form-group" id="selectTopics">
			 ...
			 <?php foreach($phaseType as $p): 
				...
				<div class="form-group">
					...
					<div class="col-lg-10">
						<input type="text" class="form-control" name="<?= $p->idPhase?>PhaseStart" id="<?= $p->idPhase?>PhaseStart" value="<?= $phaseStart ?>" data-validate="required"/>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="col-lg-10">
					<select multiple class="form-control" name="meetings[]" data-validate="required" size="10">
						<?php foreach($meetings as $m):    
							...
							<option value="<?= $m->idMeeting ?>" <?= $selected ? 'selected' : '' ?>><?= date("Y-m-d H:i:s", strtotime($m->startTime)) ?> - <?= date("Y-m-d H:i:s", strtotime($m->endTime)) ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</div>
